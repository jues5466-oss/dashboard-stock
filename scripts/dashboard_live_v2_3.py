...
def fetch_stock_news(stock_id, days=7):
    """用 FinMind 抓取個股新聞"""
    try:
        dl = get_finmind_loader()
        start_date = (pd.Timestamp.now() - timedelta(days=days)).strftime('%Y-%m-%d')
        news = dl.taiwan_stock_news(stock_id=str(stock_id), start_date=start_date)
        if news is None or news.empty:
            return pd.DataFrame()
        news = news.sort_values('date', ascending=False)
        # 過濾掉 cmoney、理財寶等論壇型來源
        news = news[~news['source'].str.contains('cmoney|CMoney|理財寶', case=False, na=False)]
        return news[['date', 'title', 'source', 'link']].reset_index(drop=True)
    except Exception as e:
        return pd.DataFrame()

@st.cache_data(ttl=300, show_spinner=False)
def fetch_market_headlines(max_items=15):
    """用 UDN RSS 抓取全市場財經頭條"""
    try:
        resp = requests.get(
            'https://money.udn.com/rssfeed/news/1001',
        ...
        for item in items[:max_items]:
            ...
            results.append({
                ...
                'description': desc.text[:100] if desc is not None and desc.text else ''
            })
...
def detect_sentiment(title):
    """簡單關鍵字情緒判斷"""
    title = title.lower()
    bullish = any(k in title for k in ['漲', '漲升', '創高', '利多', '看好', '買超', '大增', '突破', '上漲', '反彈', '回升', '亮眼', '超標', '擴產', '訂單', '出貨', '佳'])
    bearish = any(k in title for k in ['跌', '重挫', '創低', '利空', '看壞', '賣超', '大減', '跌破', '下跌', '崩跌', '傳言', '調查', '關廠', '虧損', '警示'])
...
def plot_chart(df, indicator_signals, colors, clusters=[], legal_df=None, indicators=None, patterns=None, show_candlestick=True, money_dates=None):
    # money_dates: list of date strings like ['20260504', ...] when this stock was mentioned in 錢線百分百
    # 根據資料總量調整顯示
    ...
    # 📺 錢線百分百 提及日期 — 垂直線標記
    if money_dates:
        for d_str in money_dates:
            try:
                # Convert YYYYMMDD → datetime for matching
                d = pd.to_datetime(d_str, format='%Y%m%d')
                if d in df_plot['Date'].values:
                    fig.add_vline(
                        x=d, line_color='#FFD700', line_width=2,
                        line_dash='dot', annotation_text='📺', annotation_position='top'
                    )
            except Exception:
                pass
...
# 新聞面板開關（預設開啟）
show_news = st.sidebar.checkbox("📰 新聞面板", value=True)

# 新聞來源選擇
news_source = st.sidebar.radio("新聞來源", ["個股新聞 (FinMind)", "市場頭條 (UDN)", "兩者皆顯示"], horizontal=True)

# K線型態說明 (折疊)
with st.sidebar.expander("🕯️ K線型態說明"):
    st.caption("""
    **🟢 底部型態（看漲）**
    - 🔨 锤子线：下影線長，見底反轉
    - 🌙 早晨之星：三根K線，跌→整理→漲
...
# 技術指標完整說明
with st.sidebar.expander("📖 技術指標完整攻略"):
    st.markdown("""
    ## 📊 技術指標大全
    ...
    **原理：** 過去N天平均收盤價
    - MA20 短線趨勢 / MA60 中線趨勢
    - **買點：** MA20 上穿 MA60 → 多頭排列
    - **賣點：** MA20 下穿 MA60 → 空頭排列
    ...
    **原理：** 漲幅 vs 跌幅，比值越大越強
    ...
    ### 📈 MACD 指數平滑異同移動平均線
    **原理：** 12日EMA - 26日EMA，Signal=9日EMA
    - MACD > 0 且在上升 → 多頭趨勢
    - MACD < 0 且在下降 → 空頭趨勢
    - **買點：** MACD 金叉（往上穿越 Signal）且 MACD>0
    - **賣點：** MACD 死叉（往下穿越 Signal）且 MACD<0
    ...
    **原理：** 累計成交量（漲加、跌減）
    - OBV 上升 → 主力持續買進
    - OBV 下降 → 主力持續賣出
    - **買點：** OBV 突破 20 日新高（量能增強）
    - **賣點：** OBV 跌破 20 日新低（量能減弱）
    ...
    **原理：** 衡量價格波動劇烈程度
    - ATR 擴大 → 波動加劇（可能趨勢形成）
    - ATR 收縮 → 波動降低（整理格局）
    - **買點：** ATR 突破 20 日新高（波動放大，可能突破）
    - **賣點：** ATR 持續收縮（動能耗盡）

    ---

    ### 📈 DMI 趨向指標
    **原理：** +DI 趨勢上升 / -DI 趨勢下降 / ADX 趨勢強度
    - +DI > -DI → 多頭趨勢
    - -DI > +DI → 空頭趨勢
    - ADX > 25 → 趨勢明確（強於25更強）
    - **買點：** +DI 往上穿越 -DI 且 ADX 上升
    - **賣點：** -DI 往上穿越 +DI 且 ADX 上升
    ...
    | 訊號 | 說明 |
    |------|------|
    | **價量齊揚** | 股價上漲 + 成交量增加 → 確認趨勢 |
    | **低量整理後突破** | 整理時量縮，突破時放量 → 主力進場 |
    | **地量見底** | 成交量縮到歷史低點 → 可能底部反轉 |
    | **量增價跌後反彈** | 賣壓出籠但量能開始縮 → 底部不遠 |

    #### 賣點訊號
    | 訊號 | 說明 |
    |------|------|
    | **價跌量增** | 股價下跌 + 成交量大增 → 主力出貨 |
    | **量價背離** | 股價創新高但量能未能跟上 → 趨勢反轉 |
    | **高位爆量** | 股價在高檔突然爆量 → 可能賣出訊號 |
    | **反彈量縮** | 下跌後反彈時量能不足 → 缺乏上漲動能 |

    #### MA5 成交量均線
    - 短線平均成交量，判斷是否異常放量
    - VOL/MA20 > 1.5x 為異常放量訊號
    ...
    **穩健買點（多指標確認）：**
    - RSI < 30 + KD 低檔黃金交叉 + 成交量放大
    - MA20 上穿 MA60 + MACD 零軸上方金叉

    **強勢買點（動能型）：**
    - MACD > 0 + ADX > 25 + DMI+ 上升
    - ATR 突破新高 + OBV 同步上升

    **注意：** 單一指標訊號可能假訊號，建議 2-3 個指標同時確認
    """)
...
if stock_id:
    ...
    if df is not None and len(df) > 50:
        ...
        # ── 📺 錢線百分百 提及記錄（模組層級快取） ───────────────
        money_dates = []
        money_stocks = pd.DataFrame()
        try:
            csv_path = '/Volumes/AI_Drive/StockData_v2/data/money100/mentioned_stocks.csv'
            if os.path.exists(csv_path):
                _df = pd.read_csv(csv_path, dtype={'code': str})
                code_str = str(stock_id).strip().lstrip('0')
                money_stocks = _df[_df['code'].astype(str).str.lstrip('0') == code_str]
                money_dates = list(money_stocks['date'].unique())
        except Exception:
            money_stocks = pd.DataFrame()
            money_dates = []

        st.plotly_chart(plot_chart(df, indicator_signals, colors, clusters, legal_df, indicators, patterns, show_candlestick, money_dates if money_dates else None), use_container_width=True)

        # ── 📰 新聞面板 ──────────────────────────────────────────────
        if show_news:
            with st.expander("📰 新聞面板", expanded=True):
                col_stock, col_market = st.columns(2)

                # 左側：個股新聞（FinMind）
                if news_source in ["個股新聞 (FinMind)", "兩者皆顯示"]:
                    with col_stock:
                        st.markdown("**📈 個股新聞**")
                        news_df = fetch_stock_news(stock_id, days=7)
                        if news_df is not None and not news_df.empty:
                            for _, row in news_df.head(6).iterrows():
                                sentiment = detect_sentiment(row['title'])
                                date_str = str(row.get('date', ''))[:16]
                                st.markdown(
                                    f"{sentiment} [{date_str}]({row.get('link', '#')}) "
                                    f"*{row.get('source', '')}*"
                                )
                ...
                # 右側：市場頭條（UDN）
                if news_source in ["市場頭條 (UDN)", "兩者皆顯示"]:
                    with col_market:
                        st.markdown("**🌐 市場頭條**")
                        headlines_df = fetch_market_headlines(max_items=10)
                        if headlines_df is not None and not headlines_df.empty:
                            for _, row in headlines_df.head(8).iterrows():
                                sentiment = detect_sentiment(row['title'])
                                st.markdown(
                                    f"{sentiment} [{row.get('pub_date', '')}]({row.get('link', '#')})"
                                )
                                st.caption(row['title'][:80])
                                st.divider()
                        else:
        ...
        # ── 📺 錢線百分百 日期切換（最近4天） ──────────────────────
        try:
            v2_path = '/Volumes/AI_Drive/StockData_v2/data/money100/mentioned_stocks_v2.csv'
            if os.path.exists(v2_path):
                mdf = pd.read_csv(v2_path, dtype={'code': str})
                mdf['code'] = mdf['code'].astype(str).str.lstrip('0')
                available_dates = sorted(mdf['date'].unique(), reverse=True)[:4]
                if not available_dates:
                    st.info("尚無資料")
                else:
                    if "money100_date" not in st.session_state:
                        st.session_state["money100_date"] = str(available_dates[0])
                    st.markdown("**📺 錢線百分百 日期：**")
                    bcols = st.columns(len(available_dates))
                    for i, d in enumerate(available_dates):
                        d = str(d)
                        label = f"{d[4:6]}/{d[6:8]}"
                        with bcols[i]:
                            if st.button(label, key=f"mdate_{d}"):
                                st.session_state["money100_date"] = d
                                st.rerun()
                    st.divider()

                    # ── 📺 錢線百分百 今日標的（底部固定表格） ─────────
                    sel_date = str(st.session_state.get("money100_date", ""))
                    if sel_date:
                        sel_df = mdf[mdf['date'].astype(str) == sel_date].copy()
                        if not sel_df.empty:
                            st.markdown(f"### 📺 {sel_date[4:6]}/{sel_date[6:8]} 錢線百分百標的")
                            for _, row in sel_df.iterrows():
                                ...
                                with scol1:
                                    if st.button(label, key=f"m100_{sel_date}_{code}"):
